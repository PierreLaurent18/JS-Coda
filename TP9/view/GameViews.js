export default class GameView {
    ...
    clear() {
        ...
        this.ctx_sb.clearRect(0, 0, 200, 100);
    }
    ...
    drawScoreboard(game,joueur_max) {
        ...
        const ctx = this.ctx;
        const timer_game = Math.floor((Date.now() - this.startTime) / 1000);
        ...
        for (let i = 0; i<deathPlayers_length;i++) {
            if (!this.player_death.includes(deathPlayers[i])) {
                let position_finale = joueur_max-deathPlayers_length+1;
                console.log("test");
                let co_mort = 500-((deathPlayers_length+1)*20);
                this.ctx_sb.fillStyle = "gray";
                this.ctx_sb.fillText(`${position_finale}: ${deathPlayers[i].name}  -> kill efféctué ${(deathPlayers[i].lvl-1)}`,0,co_mort);

                this.player_death.push(deathPlayers[i])
            }
        }
        ...
        for (let i=0; i<deathPlayers.length;i++) {
            
        }
        if (alivePlayers_lenght === 1 && timer_game >50) {
        this.ctx.textBaseline ="top";
